...
class MaketouScraper:
    ...
    async def scrape_async(self, url: str) -> Dict[str, Any]:
        """
        Scrape une page Maketou (boutique ou produit)
        """
    ...
    def _is_shop_page(self, soup: BeautifulSoup, url: str) -> bool:
        ...
        # Les URLs Maketou de boutiques contiennent généralement .mymaketou.store
        if '.mymaketou.store' in url.lower() or '/store' in url.lower():
            return True
        
        shop_indicators = [
            soup.find('div', class_=re.compile(r'shop|boutique|store|vendor', re.I)),
            soup.find('h1', string=re.compile(r'boutique|shop|vendeur|store', re.I)),
            soup.find_all('div', class_=re.compile(r'product|item|card', re.I))
        ]
        return any(shop_indicators)
    
    async def _scrape_shop(self, page, soup: BeautifulSoup, url: str) -> Dict[str, Any]:
        """Scrape les données d'une boutique Maketou"""
        data = {
            "name": "",
            "url": url,
        ...
        if not data["name"]:
            # Extraire depuis l'URL ou le titre
            title_elem = soup.find('title')
            if title_elem:
                title_text = title_elem.get_text()
                # Extraire le nom depuis le titre (ex: "LuminaHub - Ma boutique")
                if ' - ' in title_text:
        ...
        # Scraper les produits de la boutique avec plusieurs stratégies
        products_found = []
        ...
        # Stratégie 3: Scroll et recharger plusieurs fois
        ...
        for product_elem in products_found[:80]:
            try:
                ...
                print(f"Erreur extraction produit Maketou: {e}")
                continue
        ...
        if data["products"]:
            ...
            estimated_monthly_sales = len(data["products"]) * 10  # ~10 ventes/mois par produit
            data["estimated_sales"] = estimated_monthly_sales
            
            if avg_price > 0:
                revenue_min = estimated_monthly_sales * avg_price * 0.7
                revenue_max = estimated_monthly_sales * avg_price * 1.3
                data["estimated_revenue"] = f"{revenue_min:,.0f} - {revenue_max:,.0f} FCFA/mois"
            else:
                data["estimated_revenue"] = f"{estimated_monthly_sales * 3000:,.0f} - {estimated_monthly_sales * 12000:,.0f} FCFA/mois"
        else:
            data["estimated_sales"] = 60
            data["estimated_revenue"] = "300,000 - 900,000 FCFA/mois"
        
        # Points forts/faibles
        if data["product_count"] > 25:
            data["strengths"].append("Catalogue étendu")
        if data["product_count"] > 15:
            data["strengths"].append("Bonne diversité de produits")
        if data["product_count"] < 10:
            data["weaknesses"].append("Gamme de produits restreinte")
    ...
    def _extract_product_from_list(self, product_elem, base_url: str) -> Dict[str, Any]:
        ...
        # Prix (Maketou utilise souvent $ ou FCFA)
        ...
        for pattern in price_patterns:
            ...
            if matches:
                try:
                    ...
                    # Convertir $ en FCFA approximatif (1$ ≈ 600 FCFA)
    ...
    async def _scrape_product(self, page, soup: BeautifulSoup, url: str) -> Dict[str, Any]:
        """Scrape les données d'un produit Maketou"""
        data = {
            "name": "",
            "url": url,
            "marketplace": self.marketplace,
            ...
            if price_elem:
                ...
                if price_match:
                    try:
                        ...
                        if '$' in price_text or price_val < 100:
                            price_val = price_val * 600
        ...
        if data["price"] > 0:
            if data["price"] < 5000:
                data["estimated_daily_sales"] = 18
            elif data["price"] < 20000:
                data["estimated_daily_sales"] = 10
            else:
                data["estimated_daily_sales"] = 4
        else:
            data["estimated_daily_sales"] = 6

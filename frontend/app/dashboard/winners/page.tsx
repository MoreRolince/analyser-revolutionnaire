...
export default function WinnersPage() {
  ...
  const fetchWinners = async () => {
    ...
    try {
      ...
      // Filtrer les données par défaut (IDs 1, 2, 3 et noms "Exemple")
      ...
      const realProducts = products.filter((p: any) => {
        ...
        return p.id > 3 && 
               ...
               !name.includes('produit winner exemple')
      })
      ...
      const realShops = shops.filter((s: any) => {
        ...
        return s.id > 3 && 
               !name.includes('exemple') && 
               !name.includes('boutique winner exemple')
      })
      
      // Utiliser les vraies données si disponibles, sinon utiliser toutes les données
      setProducts(realProducts.length > 0 ? realProducts : products)
      setShops(realShops.length > 0 ? realShops : shops)
      
      console.log('Products loaded:', realProducts.length > 0 ? realProducts.length : products.length, 'total:', products.length)
      console.log('Shops loaded:', realShops.length > 0 ? realShops.length : shops.length, 'total:', shops.length)
    } catch (error: any) {
